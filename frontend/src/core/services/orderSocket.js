...
// Audit fix M9: socket lifecycle AND delivery-OTP event payloads were
// logged unconditionally, including in production builds — putting OTP
// codes and other event payloads into the console of any user's browser.
...
export function getOrderSocket(getToken) {
  ...
  if (!socket) {
    socketUrl = url;
    devLog('[orderSocket] Creating new Socket.IO connection to:', url);
    ...
    s.on("connect", () => {
      devLog("[orderSocket] Socket connected, ID:", s.id);
    ...
    s.on("disconnect", (reason) => {
      devLog("[orderSocket] Socket disconnected, reason:", reason);
    });
  ...
  if (socket.auth?.token !== token) {
    // Audit fix H4: the previous session's room lists lived on the socket
    // instance and were never cleared on a token change, so the "connect"
    // handler above would re-join every order/ticket room the PREVIOUS
    // account/role was watching, now using the NEW session's connection —
    // e.g. switching from a customer session to a seller session in the
    // same tab would re-subscribe the seller connection to the customer's
    // old order rooms. Individual pages still call joinOrderRoom/
    // joinTicketRoom for whatever they need once mounted.
    socket._orderRooms = [];
    socket._ticketRooms = [];
    socket.auth = { token };
...
export function onDeliveryOtpGenerated(getToken, handler) {
  ...
  devLog('[orderSocket] Registering delivery:otp:generated listener');

  const wrappedHandler = (payload) => {
    devLog('[orderSocket] delivery:otp:generated event received');
    handler(payload);
  };

  s.on("delivery:otp:generated", wrappedHandler);
  return () => {
    devLog('[orderSocket] Unregistering delivery:otp:generated listener');
...
export function onDeliveryOtpValidated(getToken, handler) {
  ...
  devLog('[orderSocket] Registering delivery:otp:validated listener');

  const wrappedHandler = (payload) => {
    devLog('[orderSocket] delivery:otp:validated event received');
    handler(payload);
  };

  s.on("delivery:otp:validated", wrappedHandler);
  return () => {
    devLog('[orderSocket] Unregistering delivery:otp:validated listener');
