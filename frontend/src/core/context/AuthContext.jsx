...
import { disconnectOrderSocket } from '@core/services/orderSocket';
...
export const AuthProvider = ({ children }) => {
    ...
    const logout = async () => {
        ...
        // Audit fix H4: getWithDedupe's cache is keyed only by url+params,
        // with no auth/user dimension, and is a process-wide singleton
        // shared across all four portals. Without this, a different
        // account logging back in within the cache TTL (same tab) could
        // have a request served from the previous account's cached
        // response instead of hitting the network with the new token.
        clearAllCache();

        // Audit fix H4: the order/ticket socket is also a process-wide
        // singleton that was never disconnected on logout — it stayed
        // connected with the now-invalid token indefinitely. Disconnecting
        // is safe even for multi-portal browsing: only one role renders at
        // a time per tab, and the next `getOrderSocket(...)` call (from
        // whichever portal the user navigates to next) recreates a fresh,
