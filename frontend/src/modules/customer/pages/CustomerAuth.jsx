...
const CustomerAuth = () => {
    ...
    // Lock modal size to the initial viewport so the soft keyboard cannot shrink it.
    useEffect(() => {
        const h = Math.min(Math.round(window.innerHeight * 0.85), 780);
        setCardHeightPx((prev) => prev ?? h);
    }, []);

    const liftModalForFocusedField = useCallback(() => {
        ...
        const overflow = fieldBottom - (visibleBottom - padding);

        if (overflow > 0) {
            setKeyboardLift((prev) => prev + overflow);
        }
    }, []);
    ...
    useEffect(() => {
        ...
        const onViewportChange = () => {
            ...
            window.setTimeout(liftModalForFocusedField, 50);
        };
    ...
    }, [liftModalForFocusedField]);

    const handleFieldFocus = useCallback(
        (e) => {
            e.target.style.borderColor = activeCategory.theme;
            focusTargetRef.current = e.target;
            // Wait for the soft keyboard to open, then translate the whole card up.
            window.setTimeout(liftModalForFocusedField, 300);
            window.setTimeout(liftModalForFocusedField, 550);
        },
        [activeCategory.theme, liftModalForFocusedField],
    );

    const handleFieldBlur = useCallback((e) => {
        e.target.style.borderColor =
            e.target.type === 'tel' && e.target.maxLength === 1 ? '' : '#F3F4F6';
        window.setTimeout(() => {
