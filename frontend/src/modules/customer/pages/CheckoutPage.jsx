...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    if (useWallet && user?.walletBalance) {
      ...
      // Clamp against pre-wallet total so preview+walletAmount does not feedback-loop.
      const clampBase = Number(
        pricingPreview?.grossTotal ??
          pricingPreview?.grandTotal ??
          cartTotal ??
          0,
      );
      setWalletAmountToUse(Math.min(maxAvailable, Math.max(0, clampBase)));
    } else {
  ...
  // Prefer server payableAmount. When the wallet-reduces-payable flag is off,
  // payableAmount equals the pre-wallet total and walletAmount in the
  // breakdown stays 0 — subtract locally for display only.
  const finalAmountToPay = (() => {
    const serverPayable = Number(
      pricingPreview?.payableAmount ?? pricingPreview?.grandTotal ?? 0,
    );
    const serverWalletApplied = Number(pricingPreview?.walletAmount || 0);
    if (serverWalletApplied > 0) {
      return Math.max(0, serverPayable);
    }
    return Math.max(0, serverPayable - walletAmountToUse);
  })();

  const displayDiscountAmount = Number(
    pricingPreview?.discountTotal ??
      (selectedCoupon
  ...
  useEffect(() => {
    ...
    const buildPreviewPayload = () => {
      const couponCode = selectedCoupon?.code
        ? String(selectedCoupon.code).trim()
        : null;
      const couponId =
        selectedCoupon?.couponId || selectedCoupon?._id || null;

      return {
        // Identity + qty only — server re-prices via enforceServerPricing.
        items: cart.map((item) => ({
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Server cart is the sole source of truth for line items — do not
      // send client-built items/prices/discountTotal/taxTotal.
      const orderData = {
        address: buildAddressForOrder(),
        paymentMode: selectedPayment === "online" ? "ONLINE" : "COD",
        tipAmount: selectedTip,
