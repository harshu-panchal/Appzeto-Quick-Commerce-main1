...
const DEFAULT_DELIVERY_ETA = "12-15 mins";
...
/** Legacy seed address — treat as unset so users are prompted again. */
const LEGACY_HARDCODED_ADDRESS =
...
const isUsableLocation = (loc) =>
  Boolean(loc?.name) && isValidCoords(loc?.latitude, loc?.longitude);

export const LocationProvider = ({ children }) => {
  const { settings } = useSettings();
  const deliveryEtaRef = useRef(
    settings?.estimatedDeliveryTime?.trim() || DEFAULT_DELIVERY_ETA,
  );
  ...
  useEffect(() => {
    const nextTime =
      settings?.estimatedDeliveryTime?.trim() || DEFAULT_DELIVERY_ETA;
    deliveryEtaRef.current = nextTime;
    setCurrentLocation((prev) => {
      if (!prev || prev.time === nextTime) return prev;
  ...
  useEffect(() => {
    ...
    const isLegacyHardcoded =
      typeof addressName === "string" &&
      addressName.trim() === LEGACY_HARDCODED_ADDRESS;

    if (
      parsed &&
      addressName &&
      !isLegacyHardcoded &&
      isValidCoords(parsed.latitude, parsed.longitude)
    ) {
      updateLocation(
        {
          name: addressName,
    ...
    } else if (parsed || isLegacyHardcoded) {
      // Drop expired / incomplete / legacy hardcoded cache so the user is prompted.
      remove(STORAGE_KEY);
    }
  ...
  const hasLocation = isUsableLocation(currentLocation);
  const needsLocationSetup = isLocationHydrated && !hasLocation;
