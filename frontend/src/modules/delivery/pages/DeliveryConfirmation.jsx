...
const DeliveryConfirmation = () => {
  ...
  const handleOtpValidationSuccess = (data) => {
    const shortfall = Number(data?.result?.codShortfallAmount || 0);
    if (shortfall > 0) {
      toast.warning(
        `Recorded a ₹${shortfall} shortfall against the expected COD amount. This has been flagged for admin review.`,
      );
    }
    setIsCompleted(true);
    confetti({
      ...
      <div className="flex-1 space-y-6 max-w-lg mx-auto w-full">
        ...
        {!otpGenerated ? (
          ...
          <motion.div
            ...
            transition={{ delay: 0.1, duration: 0.4 }}>
            <Card className="p-6">
              {/* Audit fix: the cash amount the rider reported collecting was
                  captured in local state but never sent to the backend —
                  a customer short-paying the rider left no record anywhere.
                  This doesn't change what the rider owes (that's a business
                  policy decision), it just makes the shortfall visible to
                  admins reconciling COD collections. */}
              <OtpInput
                orderId={orderId}
                onSuccess={handleOtpValidationSuccess}
                onError={handleOtpValidationError}
                extraPayload={!isPrepaid ? { cashCollected: Number(cashCollected) || 0 } : null}
              />
            </Card>
          </motion.div>
        )}
