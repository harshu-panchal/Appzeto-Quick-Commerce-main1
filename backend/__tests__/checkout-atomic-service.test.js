...
const mockDebitWallet = jest.fn().mockResolvedValue({});
const mockCreditWallet = jest.fn().mockResolvedValue({});
...
describe("checkout atomic service", () => {
  ...
  test("audit C-NEW-2: routes wallet redemption through canonical debitWallet even when WALLET_REDEMPTION_REDUCES_PAYABLE is unset (default/off)", async () => {
    delete process.env.WALLET_REDEMPTION_REDUCES_PAYABLE;
    mockCustomerFindById.mockReturnValue({
      session: jest.fn().mockResolvedValue({
        _id: "67f0000000000000000000c1",
        walletBalance: 1000,
    ...
    await placeOrderAtomic({
      ...
      idempotencyKey: "cccccccccccccccccccccccccccccc",
    });

    // The bug (audit C-NEW-2): with the flag off, the old code path
    // mutated `user.walletBalance` directly and never touched the
    // canonical Wallet document, so a later cancellation refund (which
    // unconditionally credits the canonical Wallet) inflated it. Asserting
    // debitWallet was called — regardless of the flag — is what proves
    // the debit and the refund now stay symmetric.
    expect(mockDebitWallet).toHaveBeenCalledTimes(1);
    expect(mockDebitWallet).toHaveBeenCalledWith(
      expect.objectContaining({ ownerType: "CUSTOMER", amount: 40 }),
    );
  });
