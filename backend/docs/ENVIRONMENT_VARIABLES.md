...
| `REDIS_DISABLED` | `false` | No | Disable Redis in any environment (queues/cache degrade) |
...
1. If Redis is **enabled**, `REDIS_URL` or `REDIS_HOST` must be set
2. `REDIS_DISABLED=true` is allowed (runs without Redis / Bull no-ops)
3. `JWT_SECRET` must not be a default/placeholder value
4. `MONGO_URI` must be set
5. With Redis disabled, prefer a single scheduler instance (no distributed lock)
