...
**Cause:** `NODE_ENV=production`, Redis is not disabled, but no `REDIS_URL` / `REDIS_HOST`.

**Fix:**
```bash
# Option A — run with Redis
...
# Option B — run without Redis (degraded: no Bull queues / shared locks)
REDIS_DISABLED=true
```
...
| Code | Meaning | Action |
|------|---------|--------|
| `REDIS_REQUIRED` | Redis enabled in production but not configured | Set `REDIS_URL` or `REDIS_DISABLED=true` |
