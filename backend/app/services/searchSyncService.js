 ...
 *
 * Audit note (2026-08): as shipped, this pipeline is scaffolding for a
 * future pluggable search backend and is NOT on the live product-search
 * path. `SEARCH_BACKEND` defaults to "mongo" (see searchService.js), whose
 * MongoSearchBackend.index()/remove() are explicit no-ops — the jobs this
 * file enqueues do real work (write a SearchIndexFailure row on repeated
 * failure) but that work has no effect on what customers see, because the
 * customer-facing product search/listing in productController.js queries
 * MongoDB directly and never calls searchService.searchProducts(). If
 * you're debugging "search results look stale," this file is a red
 * herring — look at productController.getProducts instead. Rewiring the
 * live listing endpoint through this pipeline is a deliberate follow-up
 * decision (needs a real search backend behind SEARCH_BACKEND), not
...
if (process.env.SEARCH_INDEX_ASYNC === "false") {
  // Audit fix M-search-1: this flag's name implies a sync/async toggle,
  // but there is no synchronous fallback path anywhere in the codebase —
  // setting it to "false" just skips indexing entirely (see the early
  // returns in enqueueProductIndex/enqueueProductRemoval below). Warn
  // loudly at startup so an operator who sets this expecting
  // synchronous-but-still-happening indexing isn't misled into thinking
  // indexing is occurring when it silently is not.
  logger.warn(
    "[SearchSync] SEARCH_INDEX_ASYNC=false disables search indexing entirely " +
      "(there is no synchronous fallback) — products created/updated while this " +
      "is set will not be indexed.",
  );
}
