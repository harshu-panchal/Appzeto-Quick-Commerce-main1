...
export async function verifyHandoffOtpAndDeliver(deliveryId, orderId, code, cashCollected = null) {
  ...
  // Audit fix: record what the rider actually reports collecting for COD
  // orders, and flag a shortfall against the expected order total. Does
  // NOT change the amount owed in the settlement below — see the model
  // comment on codReportedByRider for why.
  const normalizedCashCollected = Number(cashCollected);
  if (
    order.paymentMode === "COD" &&
    Number.isFinite(normalizedCashCollected) &&
    normalizedCashCollected >= 0
  ) {
    const expectedTotal = Number(
      order.paymentBreakdown?.grandTotal || order.pricing?.total || 0,
    );
    updateSet["paymentBreakdown.codReportedByRider"] = normalizedCashCollected;
    updateSet["paymentBreakdown.codShortfallAmount"] = Math.max(
      0,
      Math.round((expectedTotal - normalizedCashCollected) * 100) / 100,
    );
  }
  ...
  return {
    ...
    codShortfallAmount: updated.paymentBreakdown?.codShortfallAmount || 0,
  };
}
