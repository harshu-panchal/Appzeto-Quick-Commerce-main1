...
export async function buildCheckoutPricingSnapshot({
  ...
  const normalizedTipAmount = Math.max(
    0,
    Math.min(Number(tipAmount || 0), getMaxCustomerTipAmount()),
  );

  // Audit Phase 5 (C-2): when the flag is ON, the client-supplied
  // `discountTotal` is ALWAYS discarded — even when no coupon code/id
  // is present — so customers cannot self-credit a discount by editing
  // the Network payload. Discount is only applied when the centralized
  // engine validates a coupon against the hydrated cart.
  // When the flag is OFF, the legacy client-trust path is preserved
  // bit-for-bit so rollback is an env flip.
  let effectiveDiscount = isServerSideCouponEngineEnabled()
    ? 0
  ...
  allocateCheckoutTipToSellerBreakdowns(sellerBreakdownEntries, normalizedTipAmount);
  // Audit Phase 5 (H-6): free-delivery rebate must run AFTER handling
  // (so `grossTotal` is final on the delivery axis) and BEFORE tip /
  // wallet allocation (so they clamp against the post-rebate grandTotal,
  // matching the frontend math).
