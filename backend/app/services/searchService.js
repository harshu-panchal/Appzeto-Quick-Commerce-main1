 ...
 *
 * Audit note (2026-08): `searchProducts` below is not currently called by
 * any route/controller — the live customer-facing product search/listing
 * lives in productController.js and queries MongoDB directly. See the
 * longer note in searchSyncService.js for the full picture.
 */
...
export async function searchProducts(query) {
  ...
  try {
    ...
    // Cache result. Audit fix: was a locally hardcoded constant instead of
    // going through cacheService's TTL_CONFIG, breaking the single-source-
    // of-truth pattern every other cached resource follows.
    await cacheService.set(cacheKey, result, cacheService.getTTL("search"));
