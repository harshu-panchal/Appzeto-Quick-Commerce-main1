...
export async function handleCodOrderFinance(
  ...
  try {
    ...
    const expectedCod = roundCurrency(
      order.paymentBreakdown?.grandTotal || order.pricing?.total || 0,
    );
    const codAmountGross = roundCurrency(
      amount == null ? expectedCod : amount,
    );
    if (codAmountGross <= 0) {
      throw new Error("COD collection amount must be greater than 0");
    }
    if (codAmountGross > expectedCod + 0.01) {
      const err = new Error(
        `COD collection amount cannot exceed order payable (₹${expectedCod})`,
      );
      err.statusCode = 400;
      throw err;
    }
