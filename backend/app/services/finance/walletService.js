 ...
 *
 * Audit fix C-NEW-2: `orderPlacementService.placeOrderAtomic` now routes
 * every checkout wallet redemption through `debitWallet` unconditionally
 * (no more direct `User.walletBalance -=` mutation), so this sync path is
 * the sole writer of `User.walletBalance` on both the debit (checkout) and
 * credit (cancellation refund) side. That symmetry is what keeps
 * `Wallet.availableBalance` and `User.walletBalance` from drifting apart
 * on a redeem-then-cancel cycle — see the comment at the checkout wallet
 * debit call site for the full incident writeup.
 *
