...
function getSmsIndiaConfig() {
  return {
    ...
    // Audit fix: defaulted to HTTPS. OTP codes and the API key were
    // previously sent in plaintext over HTTP by default, exposing both to
    // anyone on the network path. Override via SMS_INDIA_HUB_URL if the
    // provider genuinely doesn't support TLS on this host.
    url: String(
