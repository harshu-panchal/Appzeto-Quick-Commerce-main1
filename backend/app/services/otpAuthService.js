...
export async function issueCustomerOtp({
  ...
  if (flow === "signup" && customer?.isVerified) {
    // A signup OTP must never be issuable against a phone number that
    // already belongs to a verified account — otherwise the signup flow
    // becomes an alternate login path that bypasses the account-existence
    // check the "login" branch below already enforces, letting anyone who
    // knows a victim's phone number request and verify an OTP for the
    // victim's existing account. Mirror the login branch's anti-enumeration
    // response in real-SMS mode; allow mock/dev mode through unchanged so
    // local testing still works end-to-end.
    if (useRealSMS()) {
      otpAuditLog("customer_otp_signup_existing_account_generic_response", {
  ...
  if (useRealSMS()) {
    try {
      ...
      // Audit fix: `otpLastSentAt` was persisted before the SMS dispatch
      // was awaited, so a transient provider failure left the resend
      // cooldown armed even though nothing was actually delivered —
      // the customer got an error AND had to wait out the cooldown before
      // they could even retry. Revert the cooldown timestamp so a failed
      // send doesn't block an immediate retry.
      customer.otpLastSentAt = lastSentAt;
      await customer.save();
      throw dispatchError;
    }
