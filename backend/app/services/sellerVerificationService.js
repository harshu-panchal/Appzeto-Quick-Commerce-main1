...
export async function issueSellerVerificationOtp({
  ...
  const previousLastSentAt = session?.lastSentAt || null;
  if (previousLastSentAt) {
    const elapsedMs = now.getTime() - new Date(previousLastSentAt).getTime();
  ...
  try {
    ...
    // Audit fix: `lastSentAt` was persisted before the OTP was actually
    // dispatched, so a transient email/SMS provider failure left the
    // resend cooldown armed even though nothing was delivered. Revert it
    // so a failed send doesn't block an immediate retry.
    session.lastSentAt = previousLastSentAt;
    await session.save();
    throw dispatchError;
  }
...
export async function issueSellerResetOtp({
  ...
  try {
    ...
    // Audit fix: see issueSellerVerificationOtp above — revert the cooldown
    // timestamp on a failed dispatch so it doesn't block an immediate retry.
    session.lastSentAt = previousLastSentAt;
    await session.save();
    throw dispatchError;
  }
