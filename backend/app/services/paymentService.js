...
async function handleOrderSideEffectsFromPaymentStatus(payment, nextStatus, reason) {
  ...
  if (nextStatus === PAYMENT_STATUS.FAILED || nextStatus === PAYMENT_STATUS.CANCELLED) {
    ...
    // Audit fix C-NEW-3: this branch used to stop at the status flip above,
    // so a wallet redemption taken at order placement was never reversed
    // when the online payment itself failed/was cancelled — the customer's
    // redeemed wallet amount was silently lost with no compensating credit.
    // Route through the same `compensateOrderCancellation` chokepoint every
    // other cancellation path (seller timeout, delivery timeout, customer
    // cancel, auto-cancel job) already uses, so the wallet/ledger reversal
    // is idempotent and consistent regardless of which path cancelled the
    // order. Stock was already released above (inside the transaction);
    // `compensateOrderCancellation`'s own release call is a no-op for an
    // already-RELEASED reservation, so this is safe to call unconditionally
    // for every order actually cancelled in this pass.
    for (const order of orders) {
      if (!cancelledOrderIds.some((id) => String(id) === String(order._id))) continue;
      try {
        await compensateOrderCancellation(order, order.orderId, {
          reason: reason || "Payment failed",
        });
      } catch (compensationError) {
        logger.warn?.("payment FAILED/CANCELLED compensation failed", {
          scope: "handleOrderSideEffectsFromPaymentStatus",
          orderId: order.orderId,
