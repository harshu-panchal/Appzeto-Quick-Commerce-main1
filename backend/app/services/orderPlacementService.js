...
export async function placeOrderAtomic({
  ...
  try {
    ...
    const tipAmount = Math.max(
      0,
      Math.min(
        Number(normalizedPayload.tipAmount || 0),
        getMaxCustomerTipAmount(),
      ),
    );
    ...
    //
    // Audit Phase 4 (C-1 + H-5) + Audit fix C-NEW-2: the debit ALWAYS routes
    // through `walletService.debitWallet`, regardless of
    // `isWalletRedemptionReducesPayableEnabled()`. That flag controls only
    // whether the wallet amount is subtracted from the customer-facing
    // `grandTotal` (a pricing decision, handled in checkoutPricingService.js
    // / pricingService.js) — it must NOT control whether the debit is
    // bookkept on the canonical `Wallet` document, because
    // `reverseOrderFinanceOnCancellation` unconditionally CREDITS the
    // canonical `Wallet` (via `creditWallet`) when an order with a wallet
    // redemption is cancelled. Previously, with the flag off, only
    // `User.walletBalance` was debited at checkout while the cancellation
    // refund credited *both* `Wallet.availableBalance` and
    // `User.walletBalance` — an asymmetric debit/credit pair that inflated
    // `Wallet.availableBalance` by the redeemed amount on every
    // redeem-then-cancel cycle, letting customers accumulate wallet balance
    // that was never actually theirs. Routing the debit through the same
    // canonical path as the credit keeps both sides symmetric no matter
    // how the order is later resolved.
    //
    // `debitWallet` writes a `LedgerEntry` (type: WALLET_PAYMENT) inside the
    // same session, debits the canonical `Wallet({ownerType:"CUSTOMER"})`
    // row, and mirrors the delta into `User.walletBalance` via `$inc`. The
    // legacy `Transaction({type:"Wallet Payment"})` row is still written for
    // backward-compat with admin dashboards that still consume the legacy
    // collection (the collection deprecation is a later phase).
    //
    // For legacy customers whose `Wallet.availableBalance` is below
    // `User.walletBalance` (their wallet was credited only on the User side
    // before Phase 4 P4-3 dual-write landed), we lazy-seed the canonical
    // Wallet up to the User value inside the same session using an
    // idempotent ledger key (`WLT-SEED-<userId>`). The seed does NOT touch
    // User.walletBalance (the User row is already correct) and is a no-op
    // for fresh customers.
    if (walletAmount > 0) {
