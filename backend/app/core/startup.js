...
async function validateDependencies() {
  ...
  // Validate Redis when enabled; REDIS_DISABLED=true is allowed in all envs
  try {
    if (isRedisEnabled()) {
      ...
      if (client && client.status === 'ready') {
        ...
        if (isProduction) {
          result.valid = false;
          result.errors.push('Redis is enabled but not ready');
        }
      }
    } else {
      result.checks.redis = {
        status: 'DISABLED',
        message: 'Redis is disabled (REDIS_DISABLED / REDIS_ENABLED)',
      };
    }
  } catch (error) {
