...
// This is the one generic, provider-agnostic upload endpoint (and the
// natural home for any future large/video upload), so it uses disk
// storage + a temp file rather than memoryStorage — the file is never
// fully buffered in Node memory, satisfying the "don't buffer large
// files in memory" requirement without touching the other, image/doc-only
// upload routes.
const diskUpload = multer({
  ...
  limits: {
    fileSize: parseInt(process.env.LOCAL_STORAGE_MAX_VIDEO_SIZE || String(100 * 1024 * 1024), 10),
  },
});
...
router.post("/upload", verifyToken, diskUpload.single("file"), async (req, res) => {
  try {
    const uploadedByModel = resolveUploadedByModel(req.user?.role);
    if (!uploadedByModel) {
      return handleResponse(res, 400, "Invalid user role for media upload");
    ...
    const resourceType = imageUpload ? "image" : videoUpload ? "video" : "raw";
    const folder = imageUpload ? "media/images" : videoUpload ? "media/videos" : "media/files";
    // Path-based: the temp file (written to disk by multer's diskStorage
    // above) is streamed straight to the provider, never fully buffered
    // in Node memory — see the diskUpload comment.
    const url = await uploadToCloudinary({ filePath: req.file.path }, folder, {
      mimeType,
      resourceType,
    });
