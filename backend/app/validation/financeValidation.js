import Joi from "joi";
import { getMaxCustomerTipAmount } from "../constants/finance.js";
...
export const checkoutPreviewSchema = Joi.object({
  ...
  // Legacy field — ignored when SERVER_SIDE_COUPON_ENGINE is on.
  // Prefer couponCode / couponId; clients must not rely on this value.
  discountTotal: Joi.number().min(0).default(0),
  taxTotal: Joi.number().min(0).default(0),
  tipAmount: Joi.number()
    .min(0)
    .max(getMaxCustomerTipAmount())
    .default(0),
...
export const createFinanceOrderSchema = checkoutPreviewSchema.keys({
  // Prefer omitting items so the server cart is the sole source of truth.
