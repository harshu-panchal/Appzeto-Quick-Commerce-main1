...
export const verifyDeliveryOtp = async (req, res) => {
  try {
    const { orderId } = req.params;
    const { code, otp, cashCollected } = req.body || {};
...
export const requestReturnPickupOtp = async (req, res) => {
  try {
    ...
    // Audit fix M-NEW: this block referenced an undefined `order` variable
    // (ReferenceError), so it always threw, was swallowed by the catch
    // below, and neither the SMS dispatch nor this duplicate socket emit
    // ever ran — despite the endpoint responding 200 "OTP sent to
    // customer". The real-time push already happens correctly inside
    // `generateReturnPickupOtp` (deliveryOtpService.js emits
    // `order:otp`/`delivery:otp:generated`), so this block now only needs
    // to fetch the order to send the SMS, which is the piece that was
    // actually missing.
    try {
      ...
      console.warn("[requestReturnPickupOtp] Notification failed:", lookupErr.message);
    }
