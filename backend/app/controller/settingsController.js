...
const updateSettingsSchema = Joi.object({
  ...
  estimatedDeliveryTime: Joi.string().allow("").trim().max(40),
  ...
  try {
    ...
    if (req.file) {
      ...
      const filter = tenantId
        ? { tenantId }
        : { $or: [{ tenantId: null }, { tenantId: { $exists: false } }] };
      const current = await Setting.findOne(filter).select("logoUrl faviconUrl").lean();
      const oldUrl = type === "favicon" ? current?.faviconUrl : current?.logoUrl;

      // Replaces (and best-effort deletes) the previous logo/favicon via
      // its own stored provider — a new capability; this used to leave
      // the old image orphaned every time it was replaced.
      const url = await replaceMedia(oldUrl, req.file.buffer, "settings", {
