  ...
  try {
    ...
    if (req.file) {
      try {
        ...
        // Audit fix: this used to only log and fall through, so a
        // Cloudinary/network blip silently created the category with no
        // image and a 200/201 response — the admin had no way to know the
        // upload failed short of noticing the missing image later. Mirror
        // updateCategory's behavior (below) and fail the request instead.
...
export const updateCategory = async (req, res) => {
  try {
    ...
    const existing = await Category.findById(id).select("type parentId image").lean();
    if (!existing) return handleResponse(res, 404, "Category not found");

    if (req.file) {
      try {
        // Replaces (and best-effort deletes) the previous image via its
        // own stored provider — a new capability; this used to leave the
        // old image orphaned on every replace.
        const url = await replaceMedia(existing.image, req.file.buffer, "categories", {
          mimeType: req.file.mimetype,
          resourceType: "image",
        });
        categoryData.image = url;
      } catch (err) {
        console.error("Upload failed for category update:", err);
