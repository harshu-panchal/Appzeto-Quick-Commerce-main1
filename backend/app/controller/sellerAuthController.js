...
const generateToken = (seller) =>
    jwt.sign(
        { id: seller._id, role: "seller", tokenVersion: seller.tokenVersion || 0 },
...
export const loginSeller = async (req, res) => {
    try {
        ...
        const seller = await Seller.findOne(query).select("+password +tokenVersion");
...
export const resetSellerPassword = async (req, res) => {
    try {
        ...
        const seller = await Seller.findOne(query).select("+tokenVersion");
        ...
        // Audit fix H5: invalidate every token issued before this reset —
        // verifyToken rejects any token whose tokenVersion doesn't match
