...
export const createOfferSection = async (req, res) => {
  try {
    ...
    // Audit fix: offer sections never invalidated their own public cache
    // (`cache:offersections:public:*`) — a create/update/delete/reorder
    // only became visible on the storefront if an unrelated product
    // mutation happened to invalidate the same key pattern in the
    // meantime. Mirrors experienceController.js's (banners/hero) pattern.
