...
export const removeFromCart = async (req, res) => {
  try {
    ...
    // Audit fix L1: previously wrote the full stack trace to a relative
    // `backend_error.log` file in the process CWD on every error
    // (overwritten each time) and logged raw request params via
    // console.log — a stray debug leftover. Use the structured logger,
    // which handles redaction/aggregation, like every other controller.
    logger.error("removeFromCart failed", { error: error.message, stack: error.stack });
    return handleResponse(res, 500, error.message);
  }
};
