...
export const replyToTicket = async (req, res) => {
    try {
        ...
        // Audit fix H1: `isAdmin` must be derived from the verified JWT role,
        // never trusted from the request body — the old code let any
        // authenticated user set `isAdmin: true` on their own request and
        // have it rendered as a support-staff reply. Ownership is enforced
        // the same way: only the ticket's own owner or an admin may reply.
        const isAdmin = req.user.role === "admin";
        if (!isAdmin && String(ticket.userId) !== String(req.user.id)) {
            return handleResponse(res, 403, "You do not have access to this ticket");
        }
