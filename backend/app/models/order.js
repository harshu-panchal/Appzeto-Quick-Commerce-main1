...
const orderSchema = new mongoose.Schema(
  {
    ...
    paymentBreakdown: {
      ...
      // Audit fix: the rider's self-reported cash-in-hand at delivery
      // confirmation was never sent to the backend at all — a customer
      // short-paying the rider left no record anywhere, and the platform's
      // cash-to-remit ledger still expected the full order total with no
      // visibility into the shortfall. These do NOT change what the rider
      // owes (that's a business-policy decision, not fixed here) — they
      // give admins a record to work from when reconciling/disputing.
      codReportedByRider: { type: Number, default: null },
      codShortfallAmount: { type: Number, default: 0 },
