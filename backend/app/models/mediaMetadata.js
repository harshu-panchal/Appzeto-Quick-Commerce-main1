...
const mediaMetadataSchema = new mongoose.Schema(
  {
    ...
    // Cloudinary-only identifier. Local-provider records leave this null
    // and are looked up via `objectKey` instead (see below) — required is
    // conditional so the local provider doesn't need a fake Cloudinary id.
    publicId: {
      type: String,
      required: function () {
    ...
    // Not required: the server-mediated buffer-upload flow (product,
    // category, banner, and settings images; seller/delivery KYC
    // documents captured during registration, before an authenticated
    // session exists) has no owning user to attribute uploads to. The
    // signed-upload-intent flow still always populates these (enforced
    // in mediaService.createUploadIntent, not at the schema layer).
    uploadedBy: {
    ...
    entityType: {
      ...
      trim: true,
      lowercase: true,
      // "settings" added for the configurable media storage feature
      // (logo/favicon uploads via settingsController.uploadSettingsImage).
      enum: ['product', 'profile', 'category', 'offer', 'banner', 'document', 'settings', 'other']
    },
    entityId: {
      type: mongoose.Schema.Types.ObjectId
    },
