...
const sellerSchema = new mongoose.Schema(
  {
    ...
    // Audit fix H5: incremented on every password reset. Embedded in the
    // JWT at login and checked on every request by `verifyToken` so a
    // password reset immediately invalidates every token issued before it.
    tokenVersion: {
      type: Number,
      default: 0,
      select: false,
